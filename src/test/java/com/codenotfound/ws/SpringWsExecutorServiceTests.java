...
public class SpringWsExecutorServiceTests {
  ...
  public void testMarshalSendAndReceive() {
    ...
    for (int request=1; request<=10; request++) {
      responses.add(helloWorldClientService.marshalSendAndReceive(new WsRequest("John-" + request, "Doe-" + request)));
    }
  ...
  public void testMarshalSendAndReceiveThread() {
    Runnable task = () -> {
      try {
        ...
        logger.info(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ");
        Future<String> response = helloWorldClientService.marshalSendAndReceive(new WsRequest(threadName, threadName));
